...
def main():
    ...
    parser.add_argument('--node',type=Path,default=Path(shutil.which('node')),help='Runtime used for installed SDK tests and consumers; npm remains build tooling')
    ...
    def run(command,cwd):
        ...
        if command[0]=='node':command[0]=str(args.node.absolute())
    ...
    run(['node','client.test.cjs'],consumer)
    ...
    run(['npm','install','--ignore-scripts','--no-audit','--no-fund','--save-dev','typescript@5.9.3','@types/node@18'],consumer)
    ...
    run(['node',consumer/'node_modules/typescript/bin/tsc','--strict','--noEmit','--module','NodeNext','--moduleResolution','NodeNext','--target','ES2022','smoke.cts','smoke.mts'],consumer)
    ...
    evidence['runtime']=run(['node','--version'],consumer)
