...
def check_sql_injection(file_path: str, lines: List[str], enabled: bool = True) -> List[CsharpFinding]:
    """csharp.sql-injection: string concat in SqlCommand."""
    if not enabled:
        return []
    pattern = re.compile(r'new\s+SqlCommand\s*\(\s*"[^"]*"\s*\+')
    return [
        _finding(file_path, i + 1, "csharp.sql-injection", CsharpRuleCategory.SECURITY, CsharpSeverity.ERROR,
