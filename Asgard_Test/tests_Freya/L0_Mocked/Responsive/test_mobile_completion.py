...
async def test_failure_at_each_resource_boundary_closes_context_and_browser(stage):
    ...
    page.evaluate.side_effect = [0, {"candidates": [], "limitations": []}, [], [], {"resourceCount": 0, "totalSize": 0}]
    context.new_page.return_value = page
    browser.new_context.return_value = context
    playwright = MagicMock()
    playwright.chromium.launch = AsyncMock(return_value=browser)
    ...
    with patch.object(module, "async_playwright") as factory:
        ...
        with patch("Asgard.Freya.Integration.services._url_safety.safe_goto", new=navigation):
            if stage == "serialization":
                with (
                    patch.object(module, "MobileCompatibilityReport", side_effect=ValueError("fixture serialization")),
                    pytest.raises(ValueError, match="fixture serialization"),
                ):
...
@pytest.mark.asyncio
async def test_hover_limitations_keep_findings_but_invalidate_score():
    from Asgard.Freya.Responsive.models.responsive_models import MobileCompatibilityIssue, MobileCompatibilityIssueType
    ...
    issue = MobileCompatibilityIssue(
        ...
        description="Observed fixture reveal",
        severity="moderate",
        suggested_fix="Verify touch access",
    )
    page, context, browser = AsyncMock(), AsyncMock(), AsyncMock()
    ...
    with patch("Asgard.Freya.Responsive.services.mobile_compatibility.async_playwright") as factory:
        ...
        factory.return_value.__aexit__ = AsyncMock()
        with (
            patch("Asgard.Freya.Integration.services._url_safety.safe_goto", new=AsyncMock()),
            patch(
                "Asgard.Freya.Responsive.services.mobile_compatibility.assess_hover_dependencies",
                new=AsyncMock(return_value=HoverAssessment([issue], ["unreadable_stylesheet"])),
            ),
        ):
            result = await MobileCompatibilityTester().test("https://fixture.invalid", ["iphone-14"])
    assert result.issues == [issue]
    assert not result.is_complete
    assert result.mobile_friendly_score is None
    assert result.check_outcomes["iphone-14"]["hover"].status == "incomplete"
    assert result.check_outcomes["iphone-14"]["hover"].limitations == ["unreadable_stylesheet"]
